...
def delete_atom():
    ...
    sample_space = []
    for i in choices:
      sample_space.append(AllChem.ReactionFromSmarts(i))

    return sample_space #Need to pick one to follow graph_ga
...
def append_atom():
    ...
    for new_atom in ['C', 'N', 'O', 'F', 'S', 'Cl', 'Br']:
      options.append('[*;!H0:1]>>[*:1]X'.replace('X', '-' + new_atom))
    for new_atom in ['C', 'N', 'O']:
      options.append('[*;!H0;!H1:1]>>[*:1]X'.replace('X', '=' + new_atom))
    for new_atom in ['C', 'N']:
      options.append('[*;H3:1]>>[*:1]X'.replace('X', '#' + new_atom))
    sample_space = []
    for i in options:  
      sample_space.append(AllChem.ReactionFromSmarts(i))
...
def insert_atom():
    ...
    for new_atom in ['C', 'N', 'O', 'S']:
      options.append('[*:1]~[*:2]>>[*:1]X[*:2]'.replace('X', new_atom))
    for new_atom in ['C', 'N']:
      options.append('[*;!H0:1]~[*:2]>>[*:1]=X-[*:2]'.replace('X', new_atom))
    for new_atom in ['C']:
      options.append('[*;!R;!H1;!H0:1]~[*:2]>>[*:1]#X-[*:2]'.replace('X', new_atom))
    sample_space = []
    for i in options:  
      sample_space.append(AllChem.ReactionFromSmarts(i))
...
def delete_cyclic_bond(): #only one option
    return AllChem.ReactionFromSmarts('[*:1]@[*:2]>>([*:1].[*:2])')
...
def change_atom(mol):
    ...
    for i in choices:
      for j in choices:
        sample_space.append('[X:1]>>[Y:1]'.replace('X', i).replace('Y', j))
...
def mutate(mol, mutation_rate):
    ...
    print('Mutation!')
    p = [0.15, 0.14, 0.14, 0.14, 0.14, 0.14, 0.15]
    index = np.random.choice(range(0,7), p=p)
    rxn_smarts_list = 7 * ['']
    rxn_smarts_list[0] = insert_atom()
    rxn_smarts_list[1] = change_bond_order()
    rxn_smarts_list[2] = delete_cyclic_bond()
    ...
    rxn_smarts_list[5] = change_atom(mol)
    rxn_smarts_list[6] = append_atom()
    new_mols_trial = rxn_smarts_list[index][1]
